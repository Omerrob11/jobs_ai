...
const EditJobForm = ({ job, onSubmit, onClose }) => {
  const [formData, setFormData] = useState({
    companyName: job.companyName || "",
    position: job.position || "",
    status: job.status || "הוגשו",
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      // Convert status back to Hebrew before submitting
      const dataToSubmit = {
        ...formData,
        status: formData.status, // This is English
      };

      // We need to convert  status back to Hebrew. You have mapping somewhere in your code
      ...
      dataToSubmit.status = hebrewStatus;
  ...
  return (
    ...
    >
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        <h2 className="text-xl font-bold mb-4">עריכת משרה</h2>
